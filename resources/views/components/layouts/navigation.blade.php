        ...
        <div class="relative flex items-center justify-between h-16">
            <div class="flex items-center justify-center flex-1 sm:items-stretch sm:justify-start">
                <div class="flex items-center flex-shrink-0">
                    <a href="{{ route('home') }}">
                        <!-- <svg class="w-8 h-8 text-sky-500" fill="none" width="0" height="0" stroke="currentColor"
                        ...
                        <div href="{{ route('home') }}" class="brand dark:text-white font-medium">
                            <h2>A<span>T</span></h2>
                        </div>
                    </a>
                </div>
                <div class="mx-auto">
                    <div class="flex space-x-4">
                        ...
                        <a href="{{ route('plans') }}" class="px-3 py-2 text-sm font-medium rounded-md hover:text-sky-600 dark:hover:text-white {{ request()->routeIs('plans') ? 'text-sky-600 dark:text-white' : 'text-slate-400' }}">
                            Planes
                        </a>
                        <a href="{{ route('citas') }}" class="px-3 py-2 text-sm font-medium rounded-md hover:text-sky-600 dark:hover:text-white {{ request()->routeIs('citas') ? 'text-sky-600 dark:text-white' : 'text-slate-400' }}">
                            Citas
                        </a>

                        <a href="{{ route('acerca') }}" class="px-3 py-2 text-sm font-medium rounded-md hover:text-sky-600 dark:hover:text-white {{ request()->routeIs('acerca') ? 'text-sky-600 dark:text-white' : 'text-slate-400' }}">
                            Acerca de nosotros
                        </a>
                ...
                <div class="ml-auto">
                    <div class="flex space-x-4">
                        ...
                        <form action="{{ route('logout') }}" method="POST">
                            @csrf
                            <a href="#" class="lg:px-3 py-2 text-sm font-medium rounded-md hover:text-sky-600 dark:hover:text-white text-slate-400" onclick="this.closest('form').submit()">Logout</a>
                        </form>
                        @endauth
                    </div>
                </div>
